...
program distributions_demo
    use scifort_stats, only : cauchy_cdf, dp, exponential_cdf, laplace_cdf, &
        logistic_cdf, normal_cdf, normal_ppf, uniform_cdf
    implicit none
    ...
    print '(a)', '      x       normal      uniform  exponential' // &
        '      laplace     logistic       cauchy'
    do i = 1, size(x)
        print '(f7.2,6(1x,f12.8))', x(i), normal_cdf(x(i)), &
...
end program distributions_demo
