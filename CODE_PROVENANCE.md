...
`test/beta_reference.f90`, `test/discrete_reference.f90`, and
`test/continuous_reference.f90` is generated by the corresponding scripts
under `tools/` with mpmath, which is BSD licensed;
only computed numerical values, not mpmath source, enter the repository.
