...
module scifort_burr12
    ...
    implicit none
    private
    public :: burr12_pdf, burr12_logpdf, burr12_cdf, burr12_sf
    public :: burr12_logcdf, burr12_logsf, burr12_ppf, burr12_isf
contains
    pure elemental function burr12_logpdf(x, c, d, loc, scale) result(y)
        ...
        if (.not. (valid_loc_scale(mu, sigma) .and. valid_shape(c) .and. valid_shape(d))) then
            ...
            if (z == 0.0_dp) then
                ...
                if (.not. ieee_is_finite(s)) then
                    ...
                    l = log1pexp(s)
                    if (l > 1.0_dp) then
                        if (d + 1.0_dp > huge(1.0_dp) / l) then
                            y = negative_infinity(x)
                        else
    ...
    end function burr12_logpdf

    pure elemental function burr12_pdf(x, c, d, loc, scale) result(y)
        ...
        real(dp), intent(in), optional :: scale !! positive scale parameter (default 1)
        real(dp) :: y, ly
        ly = burr12_logpdf(x, c, d, loc, scale)
    ...
    end function burr12_pdf

    pure elemental function burr12_logsf(x, c, d, loc, scale) result(y)
        ...
        if (.not. (valid_loc_scale(mu, sigma) .and. valid_shape(c) .and. valid_shape(d))) then
            ...
            lz = log(z)
            s = shape_times_log(c, lz, x)
            if (s == positive_infinity(s)) then
                y = negative_infinity(x)
            else if (s == negative_infinity(s)) then
                y = 0.0_dp
            else
                l = log1pexp(s)
                if (d > 1.0_dp) then
                    if (l > huge(1.0_dp) / d) then
                        y = negative_infinity(x)
                    else
                        y = -d * l
                    end if
    ...
    end function burr12_logsf

    pure elemental function burr12_sf(x, c, d, loc, scale) result(y)
        ...
        real(dp), intent(in), optional :: scale !! positive scale parameter (default 1)
        real(dp) :: y, ly
        ly = burr12_logsf(x, c, d, loc, scale)
        if (ieee_is_nan(ly)) then
            y = ly
        else if (ly == negative_infinity(ly)) then
            y = 0.0_dp
    ...
    end function burr12_sf

    pure elemental function burr12_cdf(x, c, d, loc, scale) result(y)
        ...
        real(dp), intent(in), optional :: scale !! positive scale parameter (default 1)
        real(dp) :: y, ls
        ls = burr12_logsf(x, c, d, loc, scale)
    ...
    end function burr12_cdf

    pure elemental function burr12_logcdf(x, c, d, loc, scale) result(y)
        real(dp), intent(in) :: x !! point x of log(P(X <= x))
        real(dp), intent(in) :: c !! positive finite first Burr shape parameter
        real(dp), intent(in) :: d !! positive finite second Burr shape parameter
        real(dp), intent(in), optional :: loc !! lower support endpoint (default 0)
    ...
    end function burr12_logcdf

    pure elemental function burr12_ppf(p, c, d, loc, scale) result(x)
        ...
        call get_loc_scale(loc, scale, mu, sigma)
        if (.not. (valid_loc_scale(mu, sigma) .and. valid_shape(c) .and. valid_shape(d)) .or. &
    ...
    end function burr12_ppf

    pure elemental function burr12_isf(p, c, d, loc, scale) result(x)
    ...
    end function burr12_isf

    pure elemental function burr_quantile(q, c, seed) result(z)
        real(dp), intent(in) :: q !! nonnegative log-tail transform
        real(dp), intent(in) :: c !! positive finite first shape parameter
